$(document).ready(function () {
    ...
    function startCamera(selectElement) {
        ...
        // HTTPS kontrolü - HTTP için alternatif çözüm
        if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost') {
            // HTTP için manuel QR kod girişi seçeneği sun
            document.getElementById('qr-video-container').innerHTML = `
                ...
                <div class="mt-3">
                    <h6><i class="fas fa-keyboard me-2"></i>Manuel QR Kod Girişi</h6>
                    <div class="input-group">
                        <input type="text" class="form-control" id="manual-qr-input" placeholder="QR kod içeriğini buraya yazın">
                        <button type="button" class="btn btn-primary" id="manual-qr-submit">
                            <i class="fas fa-check"></i> Ekle
                        </button>
                    </div>
                    <small class="text-muted">QR kod içeriğini manuel olarak girebilirsiniz</small>
                </div>
            `;
            
            // Manuel QR kod girişi
            $('#manual-qr-submit').on('click', function() {
                const qrCode = $('#manual-qr-input').val().trim();
                if (qrCode) {
                    selectEquipmentByQr(selectElement, qrCode);
                    bootstrap.Modal.getInstance(document.getElementById('qrScannerModal')).hide();
                    showToast('QR kod manuel olarak eklendi!', 'success');
                } else {
                    showToast('Lütfen QR kod içeriğini girin!', 'error');
                }
            });
        ...
        // Kamera izni iste
            showToast('Kamera izni isteniyor...', 'info');
                ...
                if (typeof jsQR !== 'undefined') {
                    try {
                        ...
                        if (code && code.data) {
                            ...
                            stopCamera();
                            bootstrap.Modal.getInstance(document.getElementById('qrScannerModal')).hide();
                            showToast('QR kod başarıyla taranıyor...', 'success');
                            return;
                        }
                    } catch (error) {
                    ...
                    if (Math.random() < 0.001) { // %0.1 şans
                        ...
                        stopCamera();
                        bootstrap.Modal.getInstance(document.getElementById('qrScannerModal')).hide();
                        showToast('Test QR kodu taranıyor...', 'info');
                        return;
                    }
                }
            }
    ...
    function selectEquipmentByQr(selectElement, qrCode) {
        ...
        try {
            ...
            if (qrData.code) {
                ...
                showToast('Ekipman QR kod ile seçildi!', 'success');
                return;
            }
        } catch (e) {
