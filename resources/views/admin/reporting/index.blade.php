            ...
            @forelse($equipmentByCategory as $category)
              @foreach($category['equipment'] as $index => $equipment)
                <tr>
                  ...
                  <td>{{ $equipment['location'] ?? 'Belirtilmemiş' }}</td>
...
function downloadFaultCSV() {
  showSnackbar('Arıza tablosu CSV olarak indiriliyor...');
}
