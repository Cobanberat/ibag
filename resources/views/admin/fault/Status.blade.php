...
<ul class="nav nav-tabs approval-tabs mb-3" id="approvalTab" role="tablist">
  <li class="nav-item" role="presentation">
    <button class="nav-link active" id="arizali-tab" data-bs-toggle="tab" data-bs-target="#arizali" type="button" role="tab">Arızalı</button>
  </li>
  <li class="nav-item" role="presentation">
    <button class="nav-link" id="gecmis-tab" data-bs-toggle="tab" data-bs-target="#gecmis" type="button" role="tab">Geçmiş İşlemler</button>
  </li>
</ul>
...
<div class="modal fade" id="faultyDetailModal" tabindex="-1" aria-labelledby="faultyDetailModalLabel" aria-hidden="true">
...
function applyFilters() {
  ...
  filteredData = faultyData.filter(d => {
    return (urgency === '' || d.aciliyet === urgency)
      && (search === '' || d.ekipman.toLowerCase().includes(search) || d.aciklama.toLowerCase().includes(search));
  });
  currentPage = 1;
  renderFaultyTable();
}
...
function renderFaultyTable() {
  ...
  if(pageData.length === 0) {
    tbody.innerHTML = `<tr><td colspan="6" class="text-center text-muted py-4">Kriterlere uygun kayıt bulunamadı.</td></tr>`;
  } else {
...
function renderFaultyPagination() {
  ...
  for(let i=1;i<=pageCount;i++) {
    pag.innerHTML += `<li class='page-item${i===currentPage?' active':''}'><a class='page-link' href='#' onclick='gotoFaultyPage(${i});return false;'>${i}</a></li>`;
  }
}
...
function showFaultyDetail(idx) {
  ...
  let html = `<div class='text-center mb-3'><img src='${d.resim}' alt='${d.ekipman}' class='img-fluid rounded shadow' style='max-height:160px;'></div>`;
  html += `<div class='mb-2'><span class='fw-bold'>Ekipman:</span> ${d.ekipman}</div>`;
  html += `<div class='mb-2'><span class='fw-bold'>Aciliyet:</span> <span class='badge ${d.aciliyet==='Acil'?'bg-danger':'bg-warning text-dark'}'>${d.aciliyet}</span></div>`;
  ...
  html += `<div class='mb-2'><span class='fw-bold'>Durum:</span> <span class='badge bg-danger'>Arızalı</span></div>`;
